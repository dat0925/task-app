...
## 集中モード（タスク詳細の全画面表示）でコメント列と属性列を約10%拡幅（2026-09-07）

「iPad・PCの全画面表示で、属性の開始時間まわりのボタンが窮屈」という報告。
原因は幅そのもの。集中モードの3ゾーンは `.fz-cols` の `grid-template-columns` で
固定幅を与えており、属性レール 296px の内側は
「ゾーン padding 28px ＋ セクション padding 24px」を引くと 242px しか残らない。
さらに開始時間の行は **時刻 input が 110px 固定** なので、
残り約90pxを `+10m` `+1h` `+3h` の3つで分け合う形になっていた（1つ約35px）。

### 変更（index.html の CSS のみ。JSとマークアップは無変更）

| 段 | 変更前（メモ／コメント／属性） | 変更後 |
|---|---|---|
| 既定（コンテナ >1180px） | 1fr / **376px** / **296px** | 1fr / **414px** / **326px** |
| ≤1180px（3列維持の段） | 1fr / **320px** / **264px** | 1fr / **352px** / **290px** |
| ≤1040px（2列。旧 ≤980px） | 1fr / **264px**（属性） | 1fr / **290px**（属性） |
| ≤860px（1列） | 変更なし | 変更なし |

**2列に落ちる閾値を 980px → 1040px に上げている。**
この段の下限は「3列を保てる最小幅」＝メモ 398px ＋ コメント 352px ＋ 属性 290px で決まるため、
側の2列を広げたぶん閾値も上げないと、メモが 338px まで潰れてしまう。
閾値を上げても iPad 横向き（コンテナ 1050〜1160px 前後）は3列のまま。

**どの幅でも側2列が以前より狭くなることはない**（1040〜1180px の帯だけは
コメントがメモの下に回るので、むしろ全幅になる）。

### 実測（Chromium でコンテナ幅を振って計測）

```
container 1400px  メモ 660  コメント 414  属性 326
container 1181px  メモ 441  コメント 414  属性 326
container 1180px  メモ 538  コメント 352  属性 290
container 1041px  メモ 399  コメント 352  属性 290
container 1040px  メモ 750  コメント 750  属性 290（2列）
container  860px  1列
```

開始時間の行のボタンは **34.7px → 40.7px（+17%）**、
下段の時刻プリセットは **37.7px → 42.7px（+13%）**。
列幅の増分が固定幅 input に食われないぶん、ボタンの改善は10%より大きく出る。

セキュリティ観点：**表示幅だけの変更。** DBスキーマ・RLS・認証・決済フローはいずれも未変更で、
Supabase / Stripe まわりのコードには触れていない。

`sw.js` の `CACHE_NAME` は `taskra-v63` → `taskra-v64`。

---

## タップが2回判定になる不具合と、日付のUTCずれを修正（2026-09-06）

「開始日・開始時間の反応がシビアすぎて、指を弾むように離さないと2回タップ判定になる」
という報告から2件。どちらも**症状が出る場所と原因の場所が離れていた**ので経緯を残す。

### 1. FAST TAP と本物の click の二重発火

`FAST TAP` は `touchend` で `el.click()` を合成してタップの反応を速くしている。
ブラウザはそのあと**本物の click も出す**ので、放っておくと1タップで2アクションになる。
`e.preventDefault()` で抑えている前提だったが、端末や押し方によっては効かない。

ではなぜ他のボタンで問題にならなかったかというと、click 委譲側にある
**350ms のデバウンスが偶然その重複を吸収していた**だけだった。
日付・時刻の調整ボタンは「意図的な連打があるから」という理由でデバウンス対象外に
なっており（`_noDebounce`）、そこだけ重複がむき出しになっていた。
だから「+1日」を1回押すと2日進む。

| 変更 | 内容 |
|---|---|
| click 委譲の先頭 | **合成クリック（`isTrusted=false`）の直後に来た本物の click を捨てる。** 時間ではなくイベントの出どころで判定するので、連打は殺さない |
| FAST TAP | `e.preventDefault()` を `_ft` 判定の**あと**に移動。合成クリックを出さないタップまで既定動作を止めてしまい、そのタップが丸ごと消えていた |
| FAST TAP | 合成を見送るときは `_fastTapEl` を外す。連打の2打目が重複ガードに巻き込まれて消えるのを防ぐ |

**`_noDebounce` はそのまま残している。** 重複はイベント側で消えるようになったので、
デバウンスに重複吸収を兼ねさせる必要がなくなった。

### 2. `toISOString()` による日付の1日ずれ

`new Date().toISOString().slice(0,10)` は **UTC** を返すため、
JST では **00:00〜09:00 のあいだ日付が1日前**になる。
`today()` はローカル基準だったので、この2つが混在して画面ごとに食い違っていた。

`ymd(date)` / `ymdShift(days)` を追加し、カレンダー上の日付として扱う箇所を置き換えた。

| 直した場所 | 症状 |
|---|---|
| `renderDashboard()` | 早朝は「期限切れ」「今日」がサイドバーのバッジと合わない |
| `parseQA()` | クイック追加で「明日」と書くと今日になる |
| `mkTask()` / 新規タスクの既定期限 | 既定の期限（+3日）が1日ずれる |
| ガントの左端日付 | 目盛りが1日ずれる |
| Reviewのアーカイブ既定日・プロジェクトの次回レビュー日 | 1日ずれる |
| CSVのファイル名・AIに渡す期間・サンプルデータ | 同上 |

**繰り返しタスクの次回日付（`Date.UTC` で計算）と `date-adj` は元から UTC で一貫していて正しい**ため、
触っていない。`toISOString()` が残っているのはその6ヶ所だけ。

検証：`TZ=Asia/Tokyo` で JST 9/7 00:30 を再現し、旧 `toISOString()` が `2026-09-06`、
新 `today()` が `2026-09-07` を返すことを確認した。

`sw.js` の `CACHE_NAME` は `taskra-v62` → `taskra-v63`。

---

## ビューヘルプからオンラインマニュアルへ導線を追加（2026-09-06）

`taskra.jp/guide/` にビュー別のオンラインマニュアル（15ページ）を新設したのに合わせ、
アプリ側の「?」ポップアップから該当ページへ飛べるようにした。マニュアル本体は
`dat0925/taskra-web` リポジトリ側にある（**アプリとLPでリポジトリが違うので注意**）。

### なぜ app.taskra.jp ではなく taskra.jp に置いたか

app 側はログイン必須のSPAで、Googlebot はログイン画面しか取得できない。
どれだけ書いても検索結果に出ないため、マニュアルは検索資産になる LP 側に置いた。
アプリからは外部リンクで飛ばす。
...
| 箇所 | 変更 |
|---|---|
| `index.html` `VIEW_GUIDE` / `guideUrl()` / `guideLinkHtml()`（新規） | ビュー名 → マニュアルURLの対応表。`nextview→next` / `completed→logbook` / `notes→note` のようにキーとページ名が違うので、URLは1ヶ所で持つ |
| `index.html` `VIEW_HELP.dashboard`（新規） | Dashboard だけ `VIEW_HELP` にエントリが無く、「?」を押しても何も出なかった。マニュアルへの導線を出すために追加 |
| `index.html` `openViewHelp()` | 表示条件の下に「くわしい使い方 →」を追加 |
| `index.html` `.vhelp-guide`（新規CSS） | 上記リンクの見た目 |
| `index.html` `gantt-help` モーダル | 本文を実装に合わせて修正＋マニュアルへのリンクを追加（下記） |
| `sw.js` | `CACHE_NAME` を `taskra-v61` → `taskra-v62` |

### ガントのヘルプ文言を修正した理由

修正前は「ガントチャートには**サブタスクを持つタスクのみ**表示されます」と書いてあったが、
`renderGantt()` が除外しているのは `!t.repeatRule`（繰り返しタスク）だけで、
**サブタスクの有無は一切見ていない**。実際にサブタスクの無いタスクも棒として表示される。
文言のほうが実装から取り残されていたので、実装に合わせて書き直した。

### 触っていないところ

認証・決済・DB・RLS には一切変更なし。追加したのは外部リンクと表示文言のみで、
新規テーブル・カラムもない。

### マニュアル側の再生成について

マニュアルのHTMLは手書きではなく `taskra-web/_build/build_guide.py` で生成している。
ビューを増やしたり画面を変えたりしたときは、あちらのリポジトリで
`python _build/build_guide.py` を実行し直す。キャプチャの撮り直し用に
`_build/demo-data.js`（DBに書き込まずに画面上のデータだけ差し替えるスクリプト）も置いてある。
...
## クイック追加の@プロジェクト指定バグ修正／サブタスクへの完了フィルタ適用（2026-08-31）

ユーザー報告2件への対応。DBスキーマ・RLS・認証・決済には触れていない。

### 1. プロジェクト・タグ指定済みなのにInboxに残るタスクがある

**症状**: Inbox画面の入力欄から「タスク名 @プロジェクト #タグ」の形式で追加すると、
一覧上はプロジェクト・タグが付いた状態で表示されるのに、そのタスクがInboxからも消えない。

**原因**: `submitQA()`（`index.html`、クイック追加欄 `qainput` の送信処理）で
`status` と `projectId` を別々のロジックで決めていた。

```js
const status=S.view==='project'?'active':'inbox';     // ← 現在の画面(view)だけを見ている
...
const _newProjId=p.projectId||(S.view==='project'?S.projId:null); // ← @メンションで指定されたプロジェクトも見る
```

Inbox画面にいながら `@プロジェクト名` を書くと、`_newProjId` にはそのプロジェクトIDが入るのに
`status` は画面がInboxだからという理由で `'inbox'` のまま保存される。Inbox一覧は
`status==='inbox'` で判定している（`getTasks()` 内）ため、`projectId` が入っていてもInboxに
表示され続けていた。

**対応**: `_newProjId` を先に確定させ、そこから `status` を導出するように順序を入れ替えた。

```js
const _newProjId=p.projectId||(S.view==='project'?S.projId:null);
const status=_newProjId?'active':'inbox';
```

他の作成経路（一括プロジェクト設定・AIによるタスク追加/更新など）は元々
「`projectId`が付くなら`status`もactiveにする」処理が入っており、今回のバグはなかった。
`submitQA()` だけがこの整合性チェックを欠いていた。

**触っていない箇所**: CSVインポート（`s-csv-task-import`）は `status` 列と `projectName` 列を
別々に読むため理論上は同種の不整合を作れるが、テンプレートの想定用途（列を揃えて使う）では
発生しないため、今回は未対応。次に触るときは同様に「`projectId`が確定したら`status`もそれに従わせる」
形に揃えるとよい。

**データ修復について**: 既にDB上に「`projectId`があるのに`status='inbox'`」のタスクが残っている
場合、コード修正だけでは直らない（この修正は新規作成時のみ効く）。該当タスクをプロジェクトの
ドロップダウンで一度選び直す（同じプロジェクトを選び直すだけでも `saveTask` 経路で `status` が
補正される）か、Supabaseで直接該当行を `UPDATE tasks SET status='active' WHERE status='inbox' AND
project_id IS NOT NULL` のようなクエリで補正する必要がある。今回は該当データの一括補正は行っていない。

### 2. プロジェクト/検索結果一覧でサブタスクに「完了を表示」フィルタが効かない

**症状**: 一覧画面右上の「完了を表示」をOFFにしても、タスク行を展開してサブタスクを見ると
完了済みのサブタスクがそのまま表示され続ける。

**原因**: `renderRow(task)`（一覧の行描画。プロジェクト・Inbox・Today・検索結果など
共通で使われる）が、サブタスク展開時に `S.tasks.filter(t=>t.parentTaskId===task.id)` の
結果を無条件でそのまま描画していた。トップレベルの一覧（`getTasks()`）は
`S.showCompleted||x.status!=='completed'` で完了済みを除外しているのに、
サブタスクの描画だけこのチェックが無かった。

**対応**: 展開表示するサブタスクの配列に同じ条件を適用。

```js
const visibleSubs=S.showCompleted?subs:subs.filter(s=>s.status!=='completed');
html+=visibleSubs.map(s=>'<div style="padding-left:28px">'+renderRow(s)+'</div>').join('');
```

- サブタスク件数バッジ（`n/m` 表示）は変更していない。これは総数に対する完了数の進捗表示なので、
  フィルタの有無に関わらず実数を出し続けるのが正しい挙動と判断した。
- `renderRow` は再帰的に自分自身を呼んで孫タスクも描画するため、この修正だけで
  何階層ネストしても同じフィルタが効く。
- `renderRow` はプロジェクト一覧・Inbox・Today/Flagged/Overdue・検索結果すべてで共用されているため、
  1箇所の修正で「プロジェクトや検索結果の一覧」の両方に効く。

### 検証したこと
- `node --check` 相当（`new Function()` によるパース）でindex.html内の全インラインscriptブロック
  （3ブロック）が構文エラーなしを確認。
- 実機・ブラウザでのクリック確認は今回未実施（ヘッドレス環境のため）。次にPCで見る人は、
  Inbox画面で `テスト @既存プロジェクト名 #既存タグ名` と入力して追加し、Inboxから消えて
  該当プロジェクトの一覧に移ることを確認してほしい。またサブタスクを持つタスクで
  1件完了させ、「完了を表示」をON/OFFして展開表示が切り替わることを確認してほしい。

### セキュリティ関連の状態
- **該当なし。** 認証・決済・個人情報に関わる変更なし。DBスキーマ・RLSともに無変更。
  変更は `index.html` のフロントエンドロジック2箇所のみ（＋配信用に `sw.js` のキャッシュバージョンを
  `v57`→`v58` に更新）。

### 次にやるべきこと
- 上記「データ修復について」に記載した、既存データの `status`/`projectId` 不整合の一括補正
  （必要なら実施者がSupabaseで対象件数を確認してからUPDATEすること）。
- CSVインポート経路の同種チェック追加（優先度は低め）。

---

## PWAのアップデート通知バナーを追加（2026-08-30）

「スマホ／iPad PWAで新バージョンに更新する術がない」という報告を受けて対応。
原因は2つ重なっていた。

1. `SW_UPDATED` 受信時に**予告なく即 `location.reload()`** していた
   → 入力中に消えるのを恐れて実質機能していなかった、または気づかれていなかった
2. ホーム画面PWAは通常のタブと違い、**バックグラウンドから復帰しても自動ではSWの更新チェックが
   走らない**ことがある（iOS Safari standaloneでよくある挙動）→ そもそも新バージョンを検知しにくい

### 対応方針の検討

常設の「更新ボタン」をヘッダーに置く案もあったが、却下した。ヘッダー右上は既に
検索・AI追加・タスク追加・ノート追加という「能動的に何かを作る」アイコン群で統一されており、
「アプリを直す」という受動的な操作を混ぜると意味のカテゴリが割れる。更新が不要な間
（ほぼ99%の時間）も表示され続けるのもノイズ。**Slack/Notion/VSCode等が採用している
「検知した時だけ出る自動バナー」方式**を採用した。

### 変更点

| 箇所 | 変更 |
|---|---|
| `index.html`（SW `message` リスナー） | `SW_UPDATED` 受信時の即 `reload()` を `showUpdateBanner()` に変更 |
| `index.html`（新規 `visibilitychange` リスナー） | PWAがフォアグラウンド復帰するたびに `registration.update()` を明示的に呼び、更新チェックの取りこぼしを防ぐ |
| `index.html`（新規関数 `showUpdateBanner()`） | 画面最上部にバナーを表示。「今すぐ更新」で `reload()`、「×」で今回だけ非表示（二重表示は`#update-banner`の存在チェックで防止） |
| CSS（新規 `.update-banner` 他） | ブランドカラー（`--accent`）の帯。`env(safe-area-inset-top)` でノッチ・ステータスバー分の余白を確保。`flex-wrap`で狭い幅でも折り返す |
| `sw.js` | キャッシュを `taskra-v56` → **`taskra-v57`**（この変更自体をユーザーに届けるため） |

- バナーは「×」で閉じても、次回アプリを再起動して新しいSWが検知されれば再度出る
  （閉じた状態を永続化していない。同一セッション中の再通知はしない設計）
- 既存の「タスクを開く」用の `OPEN_TASK` メッセージハンドラはそのまま。`SW_UPDATED` の分岐だけ差し替え
...
- 実機（iOS Safariのホーム画面PWA）でバックグラウンド→フォアグラウンド復帰時に
  バナーが実際に出るか確認する（今回はデスクトップChromeで見た目のみ確認済み）
- 次回以降デプロイのたびに `sw.js` の `CACHE_NAME` をインクリメントする運用を忘れないこと
  （このバナー自体、SW側のバージョンが上がらないと配信されない）

---

## スマホのタスク詳細フッターに「コピー」「最上部」ボタンを追加（2026-08-28）

`•••` メニューの奥にあった **「タスクのURLをコピー」「プロジェクトの最上部へ移動」** の2つが
使用頻度が高いので、下部アクションバー（`.mob-action-bar`）の**完了ボタンの右**へ昇格させた。

### デザインの判断：アイコン単独でもラベル単独でもなく「アイコン＋短いラベル」

| 案 | 判定 | 理由 |
|---|---|---|
| アイコンのみ | ✕ | 「最上部へ移動」に万国共通のアイコンは存在しない。`↑` だけだと「1つ上へ動かす」と誤解される |
| 文字ラベルのみ | △ | 意味は伝わるが、日本語は幅を食う。無彩色の文字が横に並ぶと「完了」と同格に見えて主役が消える |
| **アイコン＋短いラベル** | **◯** | アイコンが「探す速さ」、ラベルが「意味の確定」を担当する分業。iOS/Material 双方の定石 |

ダサくなるのはアイコンとラベルを**同格の大きさ**で並べたとき。ここでは
`.mob-btn-ico { font-size:13px; opacity:.8 }` でアイコンを一段落とし、ラベル（12.5px / 700）を
主役にして従属関係をつけている。

もう一つの肝は**色による階層**。ボタンが3つに増えると散らかるので、
**色が付くのは「完了」だけ**（緑 `#e8f5ec`）に固定し、追加した2つは既存の
`.mob-btn-ghost`（無彩色）に揃えた。押す前に主役が一目で分かる状態を保っている。

### 変更点

| 箇所 | 変更 |
|---|---|
| `.mob-btn-ico`（CSS 新規） | アイコン用。`font-size:13px;line-height:1;opacity:.8` |
| `_dtFoot()` の `.mob-action-bar` | 完了の右に `🔗 コピー` と `↑ 最上部` を追加 |
| `•••` メニュー（`task-more-btn`） | 昇格した2項目を**削除**（重複を避け、メニューを6→4項目に短縮） |
| `renderDrawer` 内 | `#copy-url-btn-mob` に既存の `copyTaskUrl()` を接続 |
| `sw.js` | キャッシュを `taskra-v55` → **`taskra-v56`** |

- `↑ 最上部` は `task.projectId` があるときだけ出す（プロジェクト未所属では移動先が無いため）。
  旧メニューの出し分け条件をそのまま踏襲している
- コピーは既存の `copyTaskUrl()` をそのまま再利用。`data-a` は通さず直接リスナー接続
  （デスクトップの `#copy-url-btn` と同じ扱い）
- 最上部は既存の `data-a="move-to-top"` の委譲ハンドラに乗せただけで、ロジックは無変更

### 幅の検証（Chromium / Playwright で実測）

各ボタンの実幅は `完了 47 / コピー 79 / 最上部 73 / ••• 49 / ✕ 34` px。
gap 5px×4 + 左右パディング 20px を足すと **約322px**。320 / 375 / 390 / 430px の
いずれでも `scrollWidth === clientWidth` で横あふれなし（`✕` の `margin-left:auto` が吸収）。

### セキュリティ

**該当なし。** UI（表示と既存アクションの導線）だけの変更。

- **新規/変更したテーブル**: なし（DBスキーマ・RLSともに無変更）
- **認証・決済フロー**: 変更なし。Supabase Auth / Stripe まわりのコードには一切触れていない
- **触っていない箇所**: `supabase/` 配下の Edge Function、`saveTask()` などの永続化ロジック、
  `move-to-top` のハンドラ本体（`topOrderInProject` を含む）

### 次にやるべきこと

- 実機（iOS Safari）で `↑` のレンダリングと 320px 幅での収まりを目視確認する
- デスクトップのフッターには「最上部へ移動」が無いまま（ヘッダの `🔗` のみ）。
  必要ならデスクトップ側 `.drawer-foot-desktop` にも同じ2つを足す

---

## 最大化ビューの列順を変更：メモ／コメント／属性（2026-08-24）

Phase 3 で作った3ゾーンの**並び順だけ**を変えた。器・部品・配線の設計はそのまま。

- **変更前**: 属性（左レール）／ 本文 ／ コメント（右）
- **変更後**: **メモ（左・最も広い）／ コメント（中央）／ 属性（右レール）**
- **優先度とタグを見出し帯から属性レールの最上部へ移した。** 属性なので属性の場所に置く、という
  整理。帯に残るのはタイトル（と親タスク行）だけになった

### なぜ左をメモにしたか

視線は左上から入る。読み書きの主役はメモで、次がコメント、属性は決めたら眺めるだけ。
主役を視線の入口に置き、参照物を端に寄せた。属性が右端に来ても、レール幅 296px と
中身（優先度・タグ・プロジェクト・日程）は変えていないので情報量は落ちていない。

### 実装（差分は3箇所だけ）

| 箇所 | 変更 |
|---|---|
| `.fz-cols` | `grid-template-columns` を `296px 1fr 376px` → **`minmax(0,1fr) 376px 296px`** |
| `.fz-rail` | 境界線を `border-right` → **`border-left`**（右端の列になったため） |
| コンテナクエリ | 折り返しを**3段**にした（次項） |
| `renderDrawer` / `renderNoteDrawer` | ゾーンの**DOM順**を `main → side → rail` に。`_dtPriTags` を帯から rail の先頭へ |
| `_dtPriTags(c,mode)` | `mode='rail'` を追加。296px に収まるよう**ラベル付きで縦積み**にする。省略時の出力は従来と1バイトも同じ |

`_dtPriTags` は器で体裁が変わるので、部品の中で分岐させた（呼び出し側で組み直さない）。
...
+'<div class="fz-cols">'
  ...
  +'<div class="fz-zone fz-rail">'
    +'<span class="fz-zone-label">属性</span>'
    +_dtPriTags(c,'rail')+_dtProject(c)+_dtSchedule(c)+_dtMeta(c)
  +'</div>'
+'</div>'
```
...
Phase 3 では3列すべてに `.fz-zone-label`（属性／本文／やりとり）を出していた。
列名をメモ・コメントに合わせると、**中身のセクション見出し（「メモ」「コメント」）と二重になる**。
セクション見出しはアコーディオンのトグルなので消せない。よって**ラベルを出すのは属性列だけ**にした。
属性列は中身が個別項目（優先度・タグ・プロジェクト…）で総称の見出しを持たないため、ここだけは必要。

### 確認したこと

1. **1列側（通常のドロワー）の出力が従来と完全一致。** `_dtPriTags` を旧実装と並べて node で評価し、
   タグ0件/複数件 × 優先度1〜4・未設定 × 選択なし/1件/2件の**30ケースすべて差分0**
2. インラインスクリプト3ブロックすべて `node --check` 通過
3. 実CSSを読み込んだハーネスで `getBoundingClientRect` を実測し、折り返しを数値で確認

### 折り返しは3段（iPad で2列に落ちた件の対応）

最初は「1180px 未満で2列」の2段だった。**iPad の横向きがちょうどここに入り、コメントが
メモの下へ落ちてしまう**（本人の指摘 2026-08-24）。iPad Safari の幅は 1180〜1194px で、
コンテナはモーダルの `97vw` ぶんなので 1145〜1158px。1180 を下回る。

そこで**2列へ落とす前に「列を詰めて3列を維持する」段を挟んだ。**

| コンテナ幅 | `grid-template-columns` | 並び | メモに残る幅 |
|---|---|---|---|
| 1180px 以上 | `1fr 376px 296px` | 3列 | — |
| 1180〜980px | `1fr 320px 264px` | **3列（iPad 横向きはここ）** | 409〜574px |
| 980〜860px | `1fr 264px` | 2列（コメントはメモの下・属性は右にフルハイト） | 636〜716px |
| 860px 未満 | `1fr` | 1列（メモ → コメント → 属性） | — |

コンテナ幅 1560/1300/1194/1158/1080/1048/1000/993/980/940/900/860/820px の13点で
`getComputedStyle` と `getBoundingClientRect` を実測して確認済み。

**幅を詰めるのはコメントと属性だけ**にしてある（メモは常に残りを取る `1fr`）。
読み書きの主役から幅を削らない、というのが3ゾーンの前提なので、そこは崩さない。

### 本番で確認済み（2026-08-24）

app.taskra.jp で最大化し、ブラウザ上で実測した値:

- グリッド `848.953px 376px 296px`、位置は メモ x=24 → コメント x=872 → 属性 x=1248
- 属性列の中身は上から「属性」ラベル → 優先度 → タグ → プロジェクト → スケジュール
- 見出し帯に残るラベルは「タイトル」だけ、`.fz-zone-label` は属性列の1つだけ
...
だから属性は**細いレール**へ寄せ、空いた幅をメモとコメントに渡す。
タイトルは列に入れず、**全幅の見出し帯**に置いて最初に読ませる。

> [!note] 2026-08-24 に列順を変更
> 当初は「属性（左）／本文／コメント（右）」で作った。同日中に
> **「メモ（左）／コメント（中央）／属性（右）」** へ変更し、優先度とタグは
> 見出し帯から属性レールの最上部へ移した。以下の図とコードは**変更後**のもの。
...
| タイトル  [                                      ]           |  .fz-band
+-------------------------------+-----------------+------------+
| メモ（常時展開・高さ制限なし） | コメント一覧    | 属性       |
| シークレットメモ              | （列いっぱい）  | [高] #仕事 |
| サブタスク / 添付             | 入力欄は下端固定 | プロジェクト |
|                               |                 | スケジュール |
|                               |                 | 作成/更新   |
+-------------------------------+-----------------+------------+
| 完了  •••                                          ✕         |  フッタ
...
dr.innerHTML= _fz
  ? _dtHead(c)+'<div class="drawer-body fz">'
      +'<div class="fz-band">'+_dtParentRow(c)+_dtTitleField(c)+'</div>'
      +'<div class="fz-cols">'
        +'<div class="fz-zone fz-main">'+_dtMemo(c)+_dtSubs(c)+_attSectionHTML('task',task.id)+'</div>'
        +'<div class="fz-zone fz-side">'+_dtComments(c)+'</div>'
        +'<div class="fz-zone fz-rail">'+_dtPriTags(c,'rail')+_dtProject(c)+_dtSchedule(c)+_dtMeta(c)+'</div>'
      +'</div>'
    +'</div>'+_dtFoot(c)
  : /* 従来の1列 */ ;
```
...
目標のレイアウト（集中モード）: **広い左＝メモ・サブタスク / 中央＝コメント / 細いレール＝属性**
（当初は「レール＝属性」を左に置いていたが、2026-08-24 に右へ移した）。
