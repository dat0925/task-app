...
## サブタスクにも「現在」フィルタ（開始前を除外）を適用（2026-09-11）

「サブタスクにも現在フィルタが効くようにして」という要望への対応。
2026-08-31 に「完了を表示」フィルタだけはサブタスクに効くようにしてあったが、
「現在」（開始前を除外）フィルタは残っていた。

### 症状

フィルタバーの「現在」をONにすると、トップレベルのタスクからは開始日が未来のもの
（`isNotStartedTask()` が true のもの）が消えるのに、タスク行を展開してサブタスクを見ると
開始前のサブタスクがそのまま並んでいた。「現在やるべきものだけ見る」モードとして使うと、
展開した瞬間に先の予定が混ざる状態だった。

### 原因

`renderRow(task)`（一覧の行描画。プロジェクト・Inbox・Today・Next・検索結果などで共用）の
サブタスク展開部分が、`S.showCompleted` しか見ていなかった。
...
トップレベル一覧（`getTasks()`）では `S.hideNotStarted` を適用している（`index.html` の
「`if(S.hideNotStarted&&S.view!=='inbox'&&S.view!=='completed')`」の行）のに、
サブタスクの描画だけこの条件が抜けていた。

### 対応

フィルタ条件を `getVisibleSubs(subs)` という関数に切り出し、`renderRow` から呼ぶ形にした
（`renderRow` の直前に定義）。将来フィルタを足すときはここ1箇所を直せば全階層に効く。

```js
function getVisibleSubs(subs){
  let v=S.showCompleted?subs:subs.filter(s=>s.status!=='completed');
  const applyNow=S.hideNotStarted&&S.view!=='inbox'&&S.view!=='completed'&&S.view!=='search'&&S.view!=='notes';
  if(applyNow)v=v.filter(s=>!isNotStartedTask(s));
  return v;
}
```
...
- **Inbox・完了ビューでは「現在」を適用しない。** トップレベル（`getTasks()`）の既存条件に揃えた。
  Inboxは捕捉・整理用の受信箱なので開始日で隠すとバッジ件数と合わなくなる、という既存の判断を
  サブタスク側でも踏襲している。
- **検索・メモビューでも適用しない。** この2つは「現在」フィルタのトグル自体をUIに出していない
  （`openFilterPopup()` の `_isNotesOrSearch`、フィルタバーの `showStatusBtns`）。
  画面から操作できないトグルでサブタスクが消えると原因を追えなくなるため、対象外にした。
- **タグフィルタ（`S.filterTagIds`）はサブタスクに適用していない。** サブタスクにタグを付けない
  運用が普通なので、適用するとほぼ全部消えてしまう。今回の要望の範囲外でもあるため触っていない。
- **件数バッジ（`n/m`）は総数のまま。** 進捗表示なのでフィルタで動かさないという 2026-08-31 の
  判断を維持している。
- **フィルタで隠れた件数を展開時に出すようにした。** 全部隠れると展開しても何も出ず故障に見えるため、
  `フィルタでN件のサブタスクを非表示` の一行を `sub-rows` の末尾に出す。階層ごとに出る。
- `renderRow` は再帰なので、孫・ひ孫にも同じフィルタが効く（検証済み）。

### 触っていないもの

- **タスク詳細（ドロワー／集中モード）のサブタスク一覧**（`_dtSubs` と `updateSubsOnly()`）。
  ここは閲覧ではなく編集UIで、フィルタで行が消えるとチェックも削除もできなくなるため、
  意図的に全件表示のままにしている。
- Gantt（`renderGantt`）とForecast（`renderForecast` / `getForecastOverdue`）は元々
  `S.hideNotStarted` をサブタスク含む全タスクに掛けているので変更不要。
- DBスキーマ・RLS・認証・決済・Edge Function には一切触れていない。

### 検証したこと

- index.html 内のインラインscript 3ブロックすべてを `new Function()` でパースし構文エラーなしを確認。
- `getVisibleSubs` の単体テスト（実ソースから関数を切り出して実行）。
  project / today / nextview / inbox / completed / search の各ビュー × 現在ON-OFF × 完了ON-OFF で
  期待通りの結果を確認。
- Chromium（Playwright、外部通信をブロックしたローカルモード）で `renderRow()` を実際に呼び、
  親→サブ→孫の3階層を描画して確認：
  - 現在OFF：開始前のサブ・孫が表示される
  - 現在ON：開始前のサブ・孫が消え、開始済みのものだけ残る（孫にも効いている）
  - 現在ON＋完了ON：完了サブは出るが開始前サブは出ない
  - Inbox／検索ビュー：現在ONでも開始前サブが出たまま（意図通り）
  - 非表示件数の一行が階層ごとに正しい件数で出る
- 実機での指タップ確認は未実施（ヘッドレス環境のため）。次にPCやiPadで見る人は、
  サブタスクに未来の開始日を設定したタスクを用意し、「現在」をON/OFFして展開表示が
  切り替わることを確認してほしい。

### セキュリティ関連の状態

- **該当なし。** 認証・決済・個人情報に関わる変更はない。新規/変更したテーブルなし、RLSの状態も無変更。
  変更は `index.html` のフロントエンド描画ロジック1箇所（＋ヘルパー関数の追加）と、
  配信用に `sw.js` のキャッシュバージョン `v66`→`v67` のみ。

### 次にやるべきこと

- タグフィルタをサブタスクにも効かせるかどうかは要判断（上記の理由で今回は見送り）。
  やるなら `getVisibleSubs()` に1行足すだけで全階層に効く。

---

## iPad/PCの全画面タスク詳細で「完了」の最終確認モーダルが背面に出て押せない不具合を修正（2026-09-08）

「iPadやPCビューでタスク詳細を全画面表示にして左下の完了をタップすると、最終確認のモーダルが
背面に出て押せない」という報告。

### 原因

`index.html` の CSS で、全画面タスク詳細（`.expand-overlay`）と汎用の確認モーダル
（`openConfirmModal()` が使う `.overlay`）の `z-index` が逆転していた。

| 要素 | 用途 | 修正前 z-index |
|---|---|---|
| `.expand-overlay` | iPad/PCの全画面タスク詳細（`openExpandModal()`） | 500 |
| `.overlay` | 完了確認などの汎用モーダル（`openConfirmModal()` 等） | **200** |

どちらも `document.body` 直下に `position:fixed` で追加される兄弟要素なので、
CSSのスタッキング規則では **DOM追加順ではなく `z-index` の大小だけ**で重なりが決まる。
確認モーダルは全画面タスク詳細より後に `body` へ追加されるが、`z-index` が低いぶん
常に背面に回り込み、クリックも通らなかった（全画面表示中の「完了」ボタン
＝ `data-a="complete-confirm"` → `openConfirmModal('タスクを完了', …)` の経路で発生）。

なお同じパターンの対処が `showUserMenu()` 内のLINE友だち追加モーダル（`.user-menu-popup`
の `z-index:3000` より手前に出す必要があった箇所、行4855付近）で既に個別の
インラインスタイル（`z-index:9999`）としてその場しのぎで入っていた。今回はその場しのぎではなく、
**`.overlay` の基準z-indexそのもの**を見直した。

### 修正

`.overlay` の `z-index` を `200` → `510`（`.expand-overlay` の `500` より確実に上）に変更。
`openConfirmModal()` に限らず、`.overlay` クラスを使う汎用モーダル全般（プロジェクト作成・
アップグレード案内・削除確認など）が、全画面タスク詳細の上に正しく表示されるようになる。

JS・マークアップは無変更。CSSの数値変更のみ。

### 検証

- 全画面タスク詳細（`.expand-overlay`, z-index:500）と確認モーダル（`.overlay`, z-index:510）を
  同条件で重ねた最小テストで、修正後は確認モーダル側が最前面（クリックが確認モーダルに通る）に
  なることを確認。
- 実データでの実機（iPad/PC実ブラウザ）操作確認は未実施。次に触る人は、全画面表示 →
  完了タップ → 確認モーダルのボタンが押せることを一度目視確認してほしい。

### セキュリティ

**該当なし。** 認証・決済・DBスキーマ・RLSはいずれも無変更。UIのz-index（表示重なり順）
のみの修正。

### 触っていないもの

`.expand-overlay` 自体の `z-index:500` や、`.tf-popup`(400)・`.link-popover`/`.proj-edit-popup`
(300) など他のポップアップ系z-indexは変更していない。

`sw.js` の `CACHE_NAME` は `taskra-v65` → `taskra-v66`。

---

## 【常設】リリースしたら X の投稿ドラフトを1本作る（build in public）

Taskra は「機能で正面から殴り合わない。開発の過程を出して、作者ごと知ってもらう」経路を取っている。
タスク管理は競合が厚く、機能紹介だけを並べても埋もれるため。
そこで **push が終わったら、この引き継ぎ書を更新するのと同じ回で `POSTS.md` に投稿ドラフトを1本足す。**

1. この HANDOVER.md に今回の節を書く（従来どおり）
2. `POSTS.md` の「在庫」の先頭にドラフトを1本追記する
   - 今回の節から **数字 / 報告された一言 / 却下した案** のどれか1つを選んで書く。機能の説明で終わらせない
   - 日本語140字以内（無料アカウントの上限。URLを付けるなら11.5字ぶん引く）
   - UIの変更なら**スクショ1枚が前提**。撮り方は「実データを写さない」手順に従う
   - **出さないもの**: 利用者のメールアドレスやユーザーID、Supabase の org / project ref、実データの画面、
     まだ塞いでいないセキュリティ上の穴の手口
3. **投稿・予約はしない。** 出すかどうかと日時は本人が決める
4. 投稿できたら `POSTS.md` の「投稿ログ」に1行足して、在庫から消す

ネタが出ない回もある。無理に足さなくてよい（在庫が10本前後あれば投稿の枠は埋まる）。
書き方の型そのもの（4つの投稿タイプ・NG・投稿のリズム）は第2の脳の
「開発ログをXで発信する型（build in public）」が正本。

---

## build in public の発信フローを引き継ぎ書に組み込んだ（2026-09-08）

コードは変えていない。**リリースのたびに発信ドラフトが1本たまる導線**を作った回。

### やったこと

| 箇所 | 変更 |
|---|---|
| `POSTS.md`（新規） | 投稿ドラフトの在庫と投稿ログ。過去のリリースから13本を掘り起こして初期在庫にした |
| `HANDOVER.md` | 上の「【常設】リリースしたら X の投稿ドラフトを1本作る」を追加（この節より上に常設で置く） |
| `CLAUDE.md` | 「PUSH後にやること」に `POSTS.md` への追記を1行足した |

初期在庫の13本は、この引き継ぎ書の各節から拾っている（Egress超過・1000行の打ち切り・
autosaveのガード欠如・二重発火・更新バナー・添付の有料線引き・E2EEメモ・決済の穴 ほか）。
**直した話が過半になるように選んだ。** 機能紹介より、詰まって直した過程のほうが読まれるため。

### 触っていないもの

`index.html` / `sw.js` / `supabase/` は無変更。SWキャッシュのバージョンも据え置き（配信物に変化がないため）。

### セキュリティ

**該当なし。** 認証・決済・個人情報に関わる変更なし。DBスキーマ・RLSともに無変更。
`POSTS.md` は GitHub Pages から取得できる位置に置くので、**利用者の識別子や実データを書かない**ことを
上の常設セクションにルールとして明記した。

### 次にやるべきこと

- 在庫の 01（完了トーストの不具合）から投稿を始める。投稿の可否は本人判断
- 投稿したら「投稿ログ」に反応（表示数・いいね）まで残し、どのタイプが読まれるかを2〜3週で見る

---

## 「完了トーストは出るのに完了にならない」不具合を修正（2026-09-07）

「タップして完了トーストが出るが完了になっていない。このタスクは何度でも完了タップできる」
という報告。**保存は成功していて、一覧が描き直されていなかった**のが真因。
症状（完了にならない）と原因（描画のスキップ）が離れていたので経緯を残す。

### 発生条件

**タスク詳細（ドロワ／集中モード）を開いた状態で、一覧側にあるサブタスクの
チェックボックスを押したとき。** ドロワを閉じていれば正常に動くので再現が不定期だった。

### 1. `updateSubsOnly()` の戻り値で `render()` ごと省いていた

`chk` ハンドラはサブタスクのとき、描画するかどうかを `updateSubsOnly()` の戻り値に委ねていた。

```js
if(task.parentTaskId){ if(!updateSubsOnly())render(); }
```

`updateSubsOnly()` は**ドロワ内の `#subs` を `S.taskId`（ドロワで開いているタスク）の
子だけ描き直して無条件に `true` を返す**関数。しかも `#subs` はドロワのタスク詳細
テンプレートに無条件で出力されるので、**詳細を開いていれば必ず存在する**
（サブタスク0件のタスクでも）。

結果、一覧のサブタスクを押すと：

1. `status='completed'` → `saveTask()` で**DBには正しく保存される**（「保存済み」表示）
2. `toast('… 完了 ✓')` が出る
3. `updateSubsOnly()` が true → **`render()` がスキップ** → 一覧の行と
   親の `3/5` バッジ（行描画でしか作られない）が古いまま

行が未完了のまま残るので何度でも押せる。しかも `chk` は `task.status` を見て
トグルするので、**2回押すと完了解除に戻る**。「何度押しても完了にならない」と
見えていたのはこれで、押した回数の偶奇でDBの状態が決まっていた。

**なぜ `render()` を避けていたか**：`render()` は `renderDrawer()` を含み、ドロワの
innerHTML を作り直すため、**入力途中のコメント・メモが消える**。意図は正しく、
「ドロワを組み直さない」ことと「一覧も描かない」ことを取り違えていたのが実際の不具合。

**修正**：共通ヘルパー `redrawAfterRowAction(task)` を追加し、ドロワは触らずに
一覧とサイドバーだけ描き直す。

```js
function redrawAfterRowAction(task){
  if(task.parentTaskId&&updateSubsOnly()){renderSidebar();renderContent();}
  else render();
}
```

`chk` の完了・完了解除の両方と、**同じ書き方だった `del`（サブタスク削除）**に適用した。
削除側は削除した行が一覧に残り続ける症状が出ていた。

### 2. 繰り返しタスクの次回タスクが往復で増殖

`chk` の完了側は `nextRepeat()` で次回タスクを作るが、**完了解除側は作った分を消さない**。
そのため「完了→完了解除→完了」を往復すると、完了するたびに同じ次回タスクが1件増えた。
1.の影響でサブタスクは何度も押せたので増えやすかったが、**トップレベルの繰り返し
タスクでも元から起きる**（検証で 完了→解除→完了 で3件になることを確認）。

**修正**：同じ次回タスク（`dueAt` ／ `repeatRule` ／ `title` ／ `parentTaskId` が一致し、
未完了・未アーカイブ）が既にあれば作らない。トーストは従来どおり次回日を出す。

副作用として、**タイトル・繰り返しルール・次回期限がすべて同一の別タスク**が
並んでいる場合は次回タスクが作られない。無制限に増えるより安全な側に倒した。

### 検証（Chromium。Supabaseをブロックしてローカルモードで描画ロジックを確認）

| 検証 | 修正前 | 修正後 |
|---|---|---|
| ドロワを開いて一覧のサブタスクを完了（完了済みを隠す設定） | 行が残り、バッジ 0/2 のまま | 行が消え、バッジ 1/2 |
| 同（完了済みを表示する設定） | 行が未完了表示のまま | 行が `.done` になり、バッジ 1/2 |
| ドロワのコメント入力中に一覧のサブタスクを完了 | 下書き保持（OK） | **下書き保持（維持できている）** |
| 繰り返しタスクで 完了→解除→完了 | 3件に増殖 | 2件（増えない） |
| ドロワを開いて一覧のサブタスクを削除 | 行が一覧に残る | 行が消え、バッジも追従 |

セキュリティ観点：**クライアント側の再描画とタスク生成の重複判定のみ。**
DBスキーマ・RLS・認証・決済フローはいずれも未変更。

`sw.js` の `CACHE_NAME` は `taskra-v64` → `taskra-v65`。
