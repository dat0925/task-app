...
| `@container (max-width:1180px)` | 2列時に**メモ＋属性レールを残し、コメントをメモの下**へ落とす（従来は属性レール＋本文を残す） |
...
| モーダル幅 | `grid-template-columns` | 並び |
|---|---|---|
| 1560px | `888px 376px 296px` | メモ → コメント → 属性（横3列） |
| 1100px | `804px 296px` | メモ（上）／ コメント（下）／ 属性（右にフルハイト） |
| 800px | `796px` | メモ → コメント → 属性（縦1列） |

**本番（app.taskra.jp）での目視確認は未実施。** ローカルで開くと Google サインインが必要なため、
デプロイ後に本人確認が必要。SW キャッシュは **v54** に上げてある。
